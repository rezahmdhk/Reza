...
# ========== تنظیمات ==========
BOT_TOKEN = "8844843302:AAExvt6yG0584mRgS2HdyEOgPAbfN0xVfRA"
...
class UltraDatabase:
    ...
    # ========== مدیریت اخطارها با بن بعد از ۳ اخطار (رفع باگ) ==========
    def add_warning(self, group_id, user_id, reason):
        ...
        # اطمینان از وجود کلید گروه در warnings_data
        if group_id not in user["warnings_data"]:
            user["warnings_data"][group_id] = []
        
        # افزودن اخطار جدید با زمان فعلی
        ...
        # حذف اخطارهای قدیمی‌تر از max_warn_reset
        ...
        # به‌روزرسانی تعداد کل اخطارها
        user["warnings"] = len(user["warnings_data"][group_id])
        ...
        if settings.get('auto_ban_on_three_warnings', True) and count >= 3:
            try:
                ...
                bot.send_message(group_id, f"⚠️ خطا در بن خودکار کاربر {user_id}: {e}")
...
# ========== مدیریت اعضای جدید (رفع باگ کپچا) ==========
            ...
            def captcha_timeout():
                ...
                if captcha_data and captcha_data["group_id"] == group_id:
                    try:
                        ...
                        logger.error(f"خطا در تایم‌اوت کپچا: {e}")
                    finally:
                        udb.delete_captcha(user_id)
            
            timer = threading.Timer(settings.get('captcha_timeout', 60), captcha_timeout)
            timer.daemon = True
            timer.start()
...
def captcha_or_2fa_answer(message):
    ...
    if captcha_data and captcha_data["group_id"] == group_id:
        try:
            ...
            if user_answer == captcha_data["answer"]:
                ...
                logger.info(f"کاربر {user_id} کپچا را با موفقیت پشت سر گذاشت.")
            else:
                ...
                if attempts >= max_attempts:
                    try:
                        ...
                        logger.info(f"کاربر {user_id} به دلیل شکست در کپچا اخراج شد.")
                    except Exception as e:
                        logger.error(f"خطا در اخراج کاربر {user_id} به دلیل کپچا: {e}")
                    finally:
                        udb.delete_captcha(user_id)
                else:
                    bot.reply_to(message, f"❌ پاسخ نادرست! {remaining} تلاش دیگر دارید.")
        except ValueError:
    ...
    if user["is_2fa_verified"] == 0 and user["twofa_code"]:
        if udb.verify_2fa(user_id, message.text):
            ...
            udb.verify_user(user_id)
            logger.info(f"کاربر {user_id} 2FA را با موفقیت تأیید کرد.")
        else:
